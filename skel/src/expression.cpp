...
***********************************************************************/
/* 
 * modified / extended in 2015 by Oliver Flasch, sourcewerk UG
 */

#define RCS4 "$Id: expression.cpp,v 1.27 2006/07/30 05:36:39 liberti Exp liberti $"
...
Operand::Operand(const Operand& t) : 
  oplabel(t.oplabel), opstringlabel(t.opstringlabel),
  constant(t.constant), depconstant(t.depconstant), 
...
// get operator label
string Operand::GetOpLabel(void) const { return opstringlabel; }
...
// set operator label 
void Operand::SetOpLabel(const string l) { opstringlabel = l; }
...
bool BasicExpression::operator == (const BasicExpression& t) const {
  ...
  if (IsLeaf() && t.IsLeaf()) {
	...
	return false;
      ...
      return false;
  } else if ((GetOpType() == OTHER) && (t.GetOpType() == OTHER) && (GetOpLabel() != t.GetOpLabel())) {
    // if both are different OTHER functions, they're different
    return false;
...
Expression Other(string label, list<Expression> as) {
  Expression ret;
  ret->SetCoeff(1);
  ret->SetExponent(1);
  ret->SetOpType(OTHER);
  ret->SetOpLabel(label);
  for (list<Expression>::const_iterator i = as.begin(), end = as.end(); i != end; ++i) {
    ret->AddCopyOfNode(*i);
  }
  return ret;
}
